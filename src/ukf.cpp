  ...
  */
  for (auto i = 0; i < n_sig_; i++) { 
    ...
    if (fabs(yawd) >= GT_ZERO) { /* first order kinetics for curved path*/
      ...
      auto ratio = v / yawd;
      order1st << ratio * (sin(new_yaw) - sin(yaw)), ratio * (-cos(new_yaw) + cos(yaw)),0,yawd * dt,0; 

				} else { /* first order kinetics for straight path*/
      order1st << v * cos(yaw) * dt, v * sin(yaw) * dt, 0, yawd * dt, 0;
    }

    VectorXd order2nd(n_x_);      /* second order kinetics */
    order2nd <<   dt2 * cos(yaw) * acc, dt2 * sin(yaw) * acc, dt * acc, dt2 * yawdd, dt * yawdd;
